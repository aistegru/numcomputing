...
M = np.zeros(1000)
M[:index] = Mw[T-2, :index]
M[index:] = Mr[T-2, index:]
...
# plotting the graph for figure(d)
M_new = np.linspace(20, 40, 1000)
C = np.linspace(10, 30, 1000)

value_r = u(C) + beta * interpolated(agrid, Vr[T - 2, :], M_new)
plt.plot(M_new, value_r, '.', markersize=1)
value_w = u(C) + beta * interpolated(agrid, Vw[T - 2, :], M_new)
plt.plot(M_new, value_w, '.', markersize=1)
